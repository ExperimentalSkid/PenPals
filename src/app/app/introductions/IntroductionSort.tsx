...
export default function IntroductionSort({ activeSort, activeStatus }: IntroductionSortProps) {
  ...
  return (
    <form method="get" className="flex items-center gap-2 text-sm text-black/55">
      ...
      <label htmlFor="introduction-sort" className="sr-only">{t("app.introductions.sortLabel")}</label>
      <select
      ...
      >
        <option value="newest">{t("app.introductions.newestFirst")}</option>
        <option value="oldest">{t("app.introductions.oldestFirst")}</option>
      </select>
