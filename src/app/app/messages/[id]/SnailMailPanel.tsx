...
import { cancelSnailMail, markSnailMailRead, revealSnailMailPhotos, sendSnailMail } from "@/app/app/messages/actions";
...
export type SnailMailAttachment = {
  id: string;
  letter_id: string;
  file_name: string;
  mime_type: string;
...
export default function SnailMailPanel({ conversationId, userId, letters, attachments = [], now, loadFailed = false, canCompose = false, composeBlockedReason = null, compact = false, viewerCountry = null, otherCountry = null }: { conversationId: string; userId: string; letters: SnailMailLetter[]; attachments?: SnailMailAttachment[]; now: number; loadFailed?: boolean; canCompose?: boolean; composeBlockedReason?: string | null; compact?: boolean; viewerCountry?: string | null; otherCountry?: string | null }) {
    ...
    return (
      <section className="rounded-xl border border-[#deded5] bg-[#fbfaf6] p-5 sm:p-6" aria-labelledby="snail-mail-heading">
        ...
        {latestLetter ? <div className="mt-5 border-t border-black/10 pt-4">
          ...
          {latestLetter.body_available && attachments.some((attachment) => attachment.letter_id === latestLetter.id) && (() => { const letterAttachments = attachments.filter((attachment) => attachment.letter_id === latestLetter.id); const hiddenSensitive = !latestIsMine && letterAttachments.some((attachment) => attachment.sensitive_content && !attachment.revealed); return <div className="mt-4"><p className="text-xs font-medium text-black/55">{t("app.snail.attachedPhotos")}</p>{hiddenSensitive ? <div className="mt-2 rounded-md border border-[#b05b4f]/25 bg-[#fff7f3] p-3"><p className="text-xs leading-5 text-black/65">{t("app.snail.sensitivePhotosWarning")}</p><form action={revealSnailMailPhotos} className="mt-2"><input type="hidden" name="conversation_id" value={conversationId} /><input type="hidden" name="letter_id" value={latestLetter.id} /><button className="rounded-md border border-[#b05b4f]/35 px-3 py-2 text-xs font-medium text-[#8d443b]">{t("app.snail.showSensitivePhotos")}</button></form></div> : <div className="mt-2 grid grid-cols-3 gap-2">{letterAttachments.map((attachment) => <a key={attachment.id} href={`/api/snail-mail-attachment/${encodeURIComponent(attachment.id)}`} target="_blank" rel="noreferrer" className="relative block aspect-square overflow-hidden rounded-md border border-black/10 bg-white"><Image src={`/api/snail-mail-attachment/${encodeURIComponent(attachment.id)}`} alt={attachment.file_name} fill sizes="(max-width: 640px) 30vw, 180px" unoptimized className="object-cover" /></a>)}</div>}</div>; })()}
  ...
  return (
    <section className="mt-8 border-y border-[#087456]/15 py-5" aria-labelledby="snail-mail-heading">
      ...
      {composing && <form action={sendSnailMail} className="mt-5 border-t border-black/10 pt-5">
        ...
        <div className="mt-4"><label htmlFor="snail-mail-photos" className="text-sm font-medium text-primary">{t("app.snail.photos")}</label><input id="snail-mail-photos" name="photos" type="file" multiple accept="image/jpeg,image/png,image/webp" disabled={composerBlocked} className="mt-2 block w-full text-sm text-black/60 file:mr-3 file:rounded-md file:border-0 file:bg-[#e7e9df] file:px-3 file:py-2 file:text-sm file:font-medium file:text-primary" /><p className="mt-1 text-xs text-black/45">{t("app.snail.photosHint")}</p><p className="mt-1 text-[11px] leading-4 text-black/40">{t("app.snail.photoRetention")}</p><label className="mt-3 flex items-start gap-2 text-sm leading-5 text-black/65"><input type="checkbox" name="sensitive_content" className="mt-1" /><span>{t("app.snail.sensitivePhotosLabel")}</span></label><label className="mt-2 flex items-start gap-2 text-sm leading-5 text-black/65"><input type="checkbox" name="photo_safety_confirmed" className="mt-1" /><span>{t("app.snail.photoSafetyConfirmation")}</span></label></div>
      ...
      {letters.length > 0 && <div className="mt-6 space-y-5" aria-live="polite">
        {letters.map((letter) => {
          ...
          return <article key={letter.id} className="border-t border-black/[0.08] pt-5">
            ...
            {letter.body_available && attachments.some((attachment) => attachment.letter_id === letter.id) && (() => { const letterAttachments = attachments.filter((attachment) => attachment.letter_id === letter.id); const hiddenSensitive = !isMine && letterAttachments.some((attachment) => attachment.sensitive_content && !attachment.revealed); return <div className="mt-4"><p className="text-xs font-medium text-black/55">{t("app.snail.attachedPhotos")}</p>{hiddenSensitive ? <div className="mt-2 max-w-xl rounded-md border border-[#b05b4f]/25 bg-[#fff7f3] p-4"><p className="text-sm leading-6 text-black/65">{t("app.snail.sensitivePhotosWarning")}</p><form action={revealSnailMailPhotos} className="mt-3"><input type="hidden" name="conversation_id" value={conversationId} /><input type="hidden" name="letter_id" value={letter.id} /><button className="rounded-md border border-[#b05b4f]/35 px-3 py-2 text-sm font-medium text-[#8d443b]">{t("app.snail.showSensitivePhotos")}</button></form></div> : <div className="mt-2 grid max-w-xl grid-cols-3 gap-3">{letterAttachments.map((attachment) => <a key={attachment.id} href={`/api/snail-mail-attachment/${encodeURIComponent(attachment.id)}`} target="_blank" rel="noreferrer" className="relative block aspect-square overflow-hidden rounded-lg border border-black/10 bg-white"><Image src={`/api/snail-mail-attachment/${encodeURIComponent(attachment.id)}`} alt={attachment.file_name} fill sizes="(max-width: 640px) 30vw, 180px" unoptimized className="object-cover" /></a>)}</div>}</div>; })()}
