...
export default function ConversationThread({ conversationId, userId, messages, hasOlderMessages, historyLoadFailed = false, introduction, otherName, otherUserId, initialOtherLastReadAt, messageSendBlocked = false, messageSendBlockedReason }: { conversationId: string; userId: string; messages: Message[]; hasOlderMessages: boolean; historyLoadFailed?: boolean; introduction: Introduction; otherName?: string; otherUserId?: string | null; initialOtherLastReadAt?: string | null; messageSendBlocked?: boolean; messageSendBlockedReason?: string }) {
  ...
  return (
    <section className="mt-6 overflow-hidden rounded-xl border border-[#deded5] bg-[#fbfaf6]" aria-label={t("app.messages.conversation")}>
      <div className="border-b border-black/10 bg-[#f8f6ef]/82 px-4 py-3.5 sm:px-5">
              ...
              return <div key={message.id}>
                ...
                <article id={`message-${message.id}`} className={`${groupedWithPrevious ? "mt-2" : "mt-6"} max-w-[84%] sm:max-w-[78%] ${mine ? "ml-auto text-right" : "mr-auto text-left"} ${activeSearchId === message.id ? "rounded-xl ring-2 ring-[#087456]/45 ring-offset-4 ring-offset-[#fbfaf6]" : ""}`}>
                  ...
                  <details onKeyDown={(event) => { if (event.key === "Escape") { event.preventDefault(); event.currentTarget.removeAttribute("open"); } }} className={`group relative mt-1 ${mine ? "text-right" : "text-left"}`}>
                    ...
                    <div className={`user-action-popover absolute top-6 z-10 w-64 text-left ${mine ? "right-0" : "left-0"}`}>
                      <form action={submitReport} className="space-y-2"><input type="hidden" name="target_type" value="message" /><input type="hidden" name="target_id" value={message.id} /><input type="hidden" name="return_to" value={`/app/messages/${encodeURIComponent(conversationId)}`} /><label className="block text-xs font-medium text-black/60" htmlFor={`reason-${message.id}`}>{t("app.messages.reportThis")}</label><select id={`reason-${message.id}`} name="reason" className="field w-full text-xs"><option value="spam">{t("app.reports.spam")}</option><option value="scam/fraud">{t("app.reports.scam")}</option><option value="harassment">{t("app.reports.harassment")}</option><option value="sexual/inappropriate content">{t("app.reports.sexual")}</option><option value="hate/abuse">{t("app.reports.hate")}</option><option value="fake profile/impersonation">{t("app.reports.fake")}</option><option value="underage concern">{t("app.reports.underage")}</option><option value="other">{t("app.reports.other")}</option></select><input name="details" aria-label={t("app.reports.details")} className="field w-full text-xs" placeholder={t("app.messages.reportDetailsPlaceholder")} /><button className="user-danger-button w-full min-h-9 px-3 py-2 text-xs">{t("app.reports.message")}</button></form>
                    </div>
                  </details>
                </article>
              </div>;
            })}
            {!historyLoadFailed && !liveMessages.length && <p className="user-empty-state my-4 py-12">{t("app.messages.emptyThread")}</p>}
          </div>
        </div>
