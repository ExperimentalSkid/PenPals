  ...
  return <main className="min-h-[calc(100vh-73px)] w-full bg-[#f7f5ef] px-4 py-6 text-[#16251f] sm:px-6 lg:px-8 xl:px-4 lg:py-8"><div className="mx-auto w-full max-w-[1320px]">
    ...
    <div className="mt-5 grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_355px] xl:items-start xl:gap-10">
      ...
      <aside className="min-w-0 space-y-5">
        <section className="rounded-xl border border-[#deded5] bg-[#fbfaf6] p-5 sm:p-6" aria-labelledby="about-person-heading"><p className="text-[10px] font-semibold uppercase tracking-[.18em] text-[#087456]">A little context</p><h2 id="about-person-heading" className="mt-1 font-serif text-2xl text-[#10231d]">About {otherName.split(" ")[0]}</h2>{publicProfile.bio && <p className="mt-4 line-clamp-5 text-sm leading-6 text-black/65">{publicProfile.bio}</p>}{location && <div className="mt-5 border-t border-black/10 pt-4"><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-[#3b5147]">Location</p><p className="mt-2 text-sm text-black/65">{location}</p></div>}<div className="mt-5 border-t border-black/10 pt-4"><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-[#3b5147]">Communication</p><div className="mt-2 space-y-1.5 text-sm text-black/70"><p className="flex items-center gap-2"><span aria-hidden="true" className={modeEnabled(otherCommunicationMode, "instant") ? "font-semibold text-[#087456]" : "font-semibold text-[#b05b4f]"}>{modeEnabled(otherCommunicationMode, "instant") ? "✓" : "✕"}</span><span>Instant Messaging</span><span className="sr-only">{modeEnabled(otherCommunicationMode, "instant") ? "available" : "unavailable"}</span></p><p className="flex items-center gap-2"><span aria-hidden="true" className={modeEnabled(otherCommunicationMode, "snail_mail") ? "font-semibold text-[#087456]" : "font-semibold text-[#b05b4f]"}>{modeEnabled(otherCommunicationMode, "snail_mail") ? "✓" : "✕"}</span><span>Snail Mail</span><span className="sr-only">{modeEnabled(otherCommunicationMode, "snail_mail") ? "available" : "unavailable"}</span></p></div></div><div className="mt-5 border-t border-black/10 pt-4"><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-[#3b5147]">Languages</p>{contextLanguageDetails.length ? <div className="mt-2 space-y-1 text-sm text-black/70">{contextLanguageDetails.map((language: any) => <p key={`${language.languageId}-${language.purpose ?? "unknown"}`} className="leading-5"><span className="inline-flex items-center gap-1.5"><LanguageFlag name={language.name} />{language.name}</span>{language.level ? <span className="text-black/45"> · {language.level}</span> : null}</p>)}</div> : <p className="mt-2 text-sm text-black/50">Not listed</p>}{languageCompatibility && (languageCompatibility.sharedLanguages.length > 0 || languageCompatibility.exchangeLanguages.length > 0) && <div className="mt-3 border-t border-black/[0.08] pt-3 text-xs leading-5 text-[#075d46]" aria-label="Language compatibility">{languageCompatibility.sharedLanguages.length > 0 && <p><span className="font-medium">You both speak:</span> {languageCompatibility.sharedLanguages.join(", ")}</p>}{languageCompatibility.exchangeLanguages.length > 0 && <p className={languageCompatibility.sharedLanguages.length > 0 ? "mt-1 text-black/55" : "text-[#075d46]"}><span className="font-medium">Language exchange:</span> {languageCompatibility.exchangeLanguages.join(", ")}</p>}</div>}</div>{contextInterests.length > 0 && <div className="mt-5 border-t border-black/10 pt-4"><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-[#3b5147]">Interests</p><div className="mt-3 flex flex-wrap gap-2">{contextInterests.map((interest: string) => <span key={interest} className="rounded-full border border-[#d7d0c3] bg-[#fffdfa] px-3 py-1.5 text-xs text-[#33443e]">{interest}</span>)}</div></div>}<Link href={profileHref ?? "/app/messages"} className="mt-6 inline-flex w-full items-center justify-center rounded-md border border-[#d7d0c3] px-4 py-2.5 text-sm font-medium text-[#075d46] hover:bg-white/75">View full profile <span aria-hidden="true" className="ml-2">→</span></Link></section>
