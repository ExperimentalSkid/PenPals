...
import SnailMailPanel, { type SnailMailAttachment, type SnailMailLetter } from "./SnailMailPanel";
...
export default async function Conversation({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; message?: string; reported?: string }> }) {
  ...
  const otherName = deletedOther ? t("app.messages.deletedUser") : (publicProfile.display_name || otherIdentity?.display_name || otherIdentity?.username || t("app.messages.conversation"));
  ...
  const contextLanguageDetails = (languageResult.data ?? [])
    .map((language: any) => {
      const name = relationName(language.languages);
      const rawLevel = formatLanguageProficiency(language.proficiency, language.purpose);
      const level = language.purpose === "learning"
        ? t("app.profile.learningLevel")
        : typeof language.proficiency === "string" && ["native", "fluent", "intermediate", "beginner"].includes(language.proficiency)
          ? t(`app.profile.levels.${language.proficiency}`)
          : rawLevel;
      return name ? { languageId: Number(language.language_id), purpose: language.purpose, name, level } : null;
    })
    .filter(Boolean);
  ...
  const [snailMailResult, snailMailAttachmentResult] = await Promise.all([
    db.rpc("list_snail_mail", { target_conversation: id }),
    db.rpc("list_snail_mail_attachments", { target_conversation: id }),
  ]);
  const snailMailLoadFailed = Boolean(snailMailResult.error);
  const snailMailAttachmentLoadFailed = Boolean(snailMailAttachmentResult.error);
  let conversationDataLoadFailed = messageHistoryLoadFailed || snailMailLoadFailed || snailMailAttachmentLoadFailed || pairBlockStateUnavailable;
  const snailMailLetters: SnailMailLetter[] = Array.isArray(snailMailResult.data) ? snailMailResult.data : [];
  const snailMailAttachments: SnailMailAttachment[] = Array.isArray(snailMailAttachmentResult.data) ? snailMailAttachmentResult.data : [];
  ...
  const reportControl = targetId ? <details className="mt-3"><summary className="flex min-h-10 cursor-pointer list-none items-center rounded-md px-3 py-2 text-sm font-medium text-black/60 transition hover:bg-black/[0.035] hover:text-primary">{t("app.reports.profile")}</summary><form action={submitReport} className="user-soft-panel mt-2 space-y-3 p-3"><input type="hidden" name="target_type" value="profile" /><input type="hidden" name="target_id" value={targetId} /><input type="hidden" name="return_to" value={`/app/messages/${encodeURIComponent(id)}`} /><label htmlFor="conversation-report-reason" className="sr-only">{t("app.reports.reason")}</label><select id="conversation-report-reason" name="reason" className="field w-full text-xs"><option value="spam">{t("app.reports.spam")}</option><option value="scam/fraud">{t("app.reports.scam")}</option><option value="harassment">{t("app.reports.harassment")}</option><option value="sexual/inappropriate content">{t("app.reports.sexual")}</option><option value="hate/abuse">{t("app.reports.hate")}</option><option value="fake profile/impersonation">{t("app.reports.fake")}</option><option value="underage concern">{t("app.reports.underage")}</option><option value="other">{t("app.reports.other")}</option></select><label htmlFor="conversation-report-details" className="sr-only">{t("app.reports.details")}</label><textarea id="conversation-report-details" name="details" className="field min-h-24 w-full text-xs" placeholder={t("app.profile.reportDetailsPlaceholder")} /><button className="user-danger-button w-full">{t("app.reports.profile")}</button></form></details> : null;
  ...
  return <main lang={locale} className="min-h-[calc(100vh-73px)] w-full bg-[#f7f5ef] px-4 py-6 text-primary sm:px-6 lg:px-8 xl:px-4 lg:py-8"><div className="mx-auto w-full max-w-[1320px]">
    ...
    <div className="mt-5 grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_355px] xl:items-start xl:gap-10">
      <div className="min-w-0">
        <header className="rounded-xl border border-[#deded5] bg-[#fbfaf6] px-5 py-5 sm:px-7 sm:py-6">
          <div className="flex flex-wrap items-start justify-between gap-6">
            <div className="flex min-w-0 items-start gap-4 sm:gap-5">
              {profileHref ? <Link href={profileHref} className="relative block h-20 w-20 shrink-0 overflow-hidden rounded-full border border-[#d8d3c7] bg-[#e5e9df] sm:h-[92px] sm:w-[92px]" aria-label={t("app.messages.viewPersonProfile", { name: otherName })}>{photoUrl ? <Image src={photoUrl} alt={t("app.messages.personProfilePhoto", { name: otherName })} fill sizes="(max-width: 640px) 80px, 92px" unoptimized className="object-cover" /> : <span className="flex h-full w-full items-center justify-center font-serif text-3xl text-muted">◦</span>}</Link> : <div className="relative block h-20 w-20 shrink-0 overflow-hidden rounded-full border border-[#d8d3c7] bg-[#e5e9df] sm:h-[92px] sm:w-[92px]" aria-hidden="true"><span className="flex h-full w-full items-center justify-center font-serif text-3xl text-muted">◦</span></div>}
        ...
        {!pairBlocked && !photoStateError && pendingTheirs.length > 0 && <section className="mt-4 rounded-xl border border-[#d7dcca] bg-[#f0f3eb] px-5 py-4" aria-live="polite" aria-label={t("app.messages.photoRequest", { name: otherName })}><p className="text-sm text-primary">{t("app.messages.photoRequest", { name: otherName })}</p><div className="mt-3 flex flex-wrap items-center gap-3"><Link href={profileHref ?? "/app/messages"} className="text-xs font-medium text-brand underline underline-offset-2">{t("app.messages.viewProfile")}</Link>{pendingTheirs.map((request: any) => <div key={request.id} className="flex items-center gap-2"><form action={respondPhotoAccess}><input type="hidden" name="request_id" value={request.id} /><input type="hidden" name="conversation_id" value={id} /><input type="hidden" name="decision" value="allowed" /><button className="rounded-md bg-[#087456] px-3 py-2 text-xs font-medium text-white">{t("app.messages.allow")}</button></form><form action={respondPhotoAccess}><input type="hidden" name="request_id" value={request.id} /><input type="hidden" name="conversation_id" value={id} /><input type="hidden" name="decision" value="declined" /><button className="rounded-md border border-black/10 px-3 py-2 text-xs font-medium text-black/60 hover:bg-black/[0.04]">{t("app.messages.decline")}</button></form></div>)}</div></section>}
        {conversationMode === "snail_mail" ? <section aria-labelledby="snail-mail-only-heading" className="rounded-xl border border-[#deded5] bg-[#fbfaf6] px-6 py-8 text-center sm:px-8"><h2 id="snail-mail-only-heading" className="section-title">{t("app.messages.snailExchange")}</h2><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-black/55">{t("app.messages.snailExchangeBody")}</p></section> : <ConversationThread conversationId={id} userId={uid} messages={msgs} hasOlderMessages={hasOlderMessages} historyLoadFailed={messageHistoryLoadFailed} introduction={openingIntroduction} otherName={otherName} otherUserId={targetId} initialOtherLastReadAt={otherParticipant?.last_read_at ?? null} messageSendBlocked={pairBlocked || pairBlockStateUnavailable || messageStreak >= 3} messageSendBlockedReason={pairBlocked ? "Messaging is unavailable because one of you blocked the other." : pairBlockStateUnavailable ? t("app.messages.conversationLoadError") : undefined} />}
      </div>

      <aside className="min-w-0 space-y-5">
        <section className="rounded-xl border border-[#deded5] bg-[#fbfaf6] p-5 sm:p-6" aria-labelledby="about-person-heading"><p className="eyebrow">{t("app.messages.context")}</p><h2 id="about-person-heading" className="section-title mt-1">{t("app.messages.aboutPerson", { name: otherName.split(" ")[0] })}</h2>{publicProfile.bio && <p className="mt-4 line-clamp-5 text-sm leading-6 text-black/65">{publicProfile.bio}</p>}{location && <div className="mt-5 border-t border-black/10 pt-4"><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-primary">{t("app.messages.location")}</p><p className="mt-2 text-sm text-black/65">{location}</p></div>}<div className="mt-5 border-t border-black/10 pt-4"><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-primary">{t("app.messages.communication")}</p><div className="mt-2 space-y-1.5 text-sm text-black/70"><p className="flex items-center gap-2"><span aria-hidden="true" className={modeEnabled(otherCommunicationMode, "instant") ? "font-semibold text-brand" : "font-semibold text-[#b05b4f]"}>{modeEnabled(otherCommunicationMode, "instant") ? "✓" : "✕"}</span><span>{t("app.messages.instantMessaging")}</span><span className="sr-only">{modeEnabled(otherCommunicationMode, "instant") ? t("app.messages.available") : t("app.messages.unavailable")}</span></p><p className="flex items-center gap-2"><span aria-hidden="true" className={modeEnabled(otherCommunicationMode, "snail_mail") ? "font-semibold text-brand" : "font-semibold text-[#b05b4f]"}>{modeEnabled(otherCommunicationMode, "snail_mail") ? "✓" : "✕"}</span><span>{t("app.messages.snail")}</span><span className="sr-only">{modeEnabled(otherCommunicationMode, "snail_mail") ? t("app.messages.available") : t("app.messages.unavailable")}</span></p></div></div><div className="mt-5 border-t border-black/10 pt-4"><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-primary">{t("app.messages.languages")}</p>{contextLanguageDetails.length ? <div className="mt-2 space-y-1 text-sm text-black/70">{contextLanguageDetails.map((language: any) => <p key={`${language.languageId}-${language.purpose ?? "unknown"}`} className="leading-5"><span className="inline-flex items-center gap-1.5"><LanguageFlag name={language.name} />{language.name}</span>{language.level ? <span className="text-black/45"> · {language.level}</span> : null}</p>)}</div> : <p className="mt-2 text-sm text-black/50">{t("app.messages.notListed")}</p>}{languageCompatibility && (languageCompatibility.sharedLanguages.length > 0 || languageCompatibility.exchangeLanguages.length > 0) && <div className="mt-3 border-t border-black/[0.08] pt-3 text-xs leading-5 text-brand" aria-label={t("app.messages.languageCompatibility")}>{languageCompatibility.sharedLanguages.length > 0 && <p><span className="font-medium">{t("app.messages.bothSpeak")}</span> {languageCompatibility.sharedLanguages.join(", ")}</p>}{languageCompatibility.exchangeLanguages.length > 0 && <p className={languageCompatibility.sharedLanguages.length > 0 ? "mt-1 text-black/55" : "text-brand"}><span className="font-medium">{t("app.messages.languageExchange")}</span> {languageCompatibility.exchangeLanguages.join(", ")}</p>}</div>}</div>{contextInterests.length > 0 && <div className="mt-5 border-t border-black/10 pt-4"><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-primary">{t("app.messages.interests")}</p><div className="mt-3 flex flex-wrap gap-2">{contextInterests.map((interest: string) => <span key={interest} className="rounded-full border border-[#d7d0c3] bg-[#fffdfa] px-3 py-1.5 text-xs text-primary">{interest}</span>)}</div></div>}<Link href={profileHref ?? "/app/messages"} className="mt-6 inline-flex w-full items-center justify-center rounded-md border border-[#d7d0c3] px-4 py-2.5 text-sm font-medium text-brand hover:bg-white/75">{t("app.messages.viewFullProfile")} <span aria-hidden="true" className="ml-2">→</span></Link></section>
        <SnailMailPanel compact conversationId={id} userId={uid} letters={snailMailLetters} attachments={snailMailAttachments} now={currentTimestamp()} loadFailed={snailMailLoadFailed} canCompose={!snailMailLoadFailed && !pairBlockStateUnavailable && canComposeSnailMail && !snailMailBlockedReason} composeBlockedReason={snailMailLoadFailed ? t("app.messages.snailLoadError") : pairBlockStateUnavailable ? t("app.messages.conversationLoadError") : snailMailBlockedReason} viewerCountry={ownProfileResult.data?.country ?? null} otherCountry={publicProfile.country ?? null} />
      </aside>
    </div>
  </div></main>;
}
