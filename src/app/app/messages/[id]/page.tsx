      ...
      <div className="min-w-0">
        <header className="rounded-xl border border-[#deded5] bg-[#fbfaf6] px-5 py-5 sm:px-7 sm:py-6">
          ...
          {targetId && <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-black/10 pt-3 text-sm">
            ...
            {!pairBlocked && !photoStateError && pendingTheirs.length === 0 && ownGrant && <div className="flex items-center gap-3 text-sm text-black/55"><span>Photo access granted to {otherIdentity?.display_name ?? "them"}</span><form action={revokePhotoAccess}><input type="hidden" name="conversation_id" value={id} /><input type="hidden" name="viewer_id" value={targetId} /><button className="text-xs text-black/45 underline">{t("app.messages.revoke")}</button></form></div>}
