    ...
    <div>
      ...
      <div className="mt-4 space-y-2">
        {selected.map((code) => {
          ...
          return <div key={code} className="flex items-center justify-between border-b border-black/10 py-2 text-sm"><span className="flex items-center gap-2"><CountryFlag code={code} countryName={country?.name ?? code} /><span>{country?.name ?? code}</span></span><button type="button" aria-label={t("app.settings.removeCountry", { name: country?.name ?? code })} onClick={() => setSelected((current) => current.filter((item) => item !== code))} className="text-xs text-black/50 hover:text-black/80">{t("app.settings.remove")}</button><input type="hidden" name="excluded_countries" value={code} /></div>;
        })}
      </div>
    </div>
  );
