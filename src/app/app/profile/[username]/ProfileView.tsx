...
function DetailHeading({ icon, children }: { icon: DetailIconName; children: ReactNode }) {
  return <h2 className="flex items-center gap-3 font-sans text-[11px] font-semibold uppercase tracking-[.18em] text-[#3b5147]"><DetailIcon name={icon} />{children}</h2>;
}
...
export default function ProfileView({ profile, displayName, age, location, activity, responseRate, photo, languages, interests, friendshipDestinations = [], personality, communicationPreference, languageCompatibility, badges, blocked, targetId, username, reportControl, isOwn, backHref = "/app/discover", backLabel = "Back to discover", reportError = null, reportSubmitted = false }: ProfileViewProps) {
  ...
  return (
    <main className="min-h-screen w-full bg-[#f7f5ef] px-5 py-6 text-[#16251f] sm:px-8 lg:px-10 lg:py-8">
      <div className="mx-auto w-full max-w-[1240px]">
        <Link href={backHref} className="inline-flex items-center gap-2 text-sm font-medium text-[#08654d] transition hover:text-[#054d3d] hover:underline"><span aria-hidden="true">←</span> {backLabel}</Link>
        {reportError && <p role="alert" className="mt-4 border-l-2 border-red-400 px-3 py-2 text-sm text-red-700">{reportError}</p>}
        {reportSubmitted && <p role="status" className="mt-4 border-l-2 border-[#087456] px-3 py-2 text-sm text-[#075d46]">Thanks for letting us know. We&apos;ll review your report.</p>}
        ...
        <section className="mt-9 grid grid-cols-1 gap-y-12 lg:grid-cols-[280px_minmax(0,1fr)] lg:gap-x-12 xl:grid-cols-[260px_minmax(0,1fr)_280px] xl:gap-x-10 2xl:grid-cols-[300px_minmax(0,1fr)_300px] 2xl:gap-x-12">
          <aside className="w-full max-w-[300px] justify-self-center lg:max-w-[280px] lg:justify-self-start xl:max-w-[260px] 2xl:max-w-[300px]">
            <div className="relative aspect-[0.68] w-full overflow-hidden rounded-[20px] border border-[#d9cdb9] bg-[#f1e8d9] p-2 shadow-[0_3px_0_#e4d8c6]">
              <div className="relative h-full w-full overflow-hidden rounded-[14px] bg-[#e9e8df]">
                {photo ? <Image src={photo} alt={displayName} fill priority sizes="(min-width: 1280px) 300px, 240px" unoptimized={isSignedAvatarUrl(photo)} className="object-cover" /> : <div role="img" aria-label={`${displayName} profile photo unavailable`} className="flex h-full items-center justify-center"><span aria-hidden="true" className="font-serif text-6xl text-[#6f806f]">{initial}</span></div>}
              </div>
              {photo && <div className="absolute bottom-4 left-1/2 inline-flex -translate-x-1/2 items-center gap-2 whitespace-nowrap rounded-full bg-[#fbfaf7]/95 px-3 py-1.5 text-xs font-medium text-[#075d46] shadow-sm"><DetailIcon name="eye" />Photo visible to you</div>}
            </div>
            {profile.is_verified && <div className="mt-3 flex justify-center">
              <ProfileBadge
                ...
                className="group"
                trailing={<>
                  <button type="button" aria-label="About profile verification" aria-describedby="profile-verification-tooltip" className="relative flex h-5 w-5 items-center justify-center rounded-full border border-[#087456]/35 text-[11px] font-semibold text-[#075d46] outline-none transition hover:bg-white/70 focus-visible:ring-2 focus-visible:ring-[#087456]/40">
                    <span aria-hidden="true">?</span>
                  </button>
                  <span id="profile-verification-tooltip" role="tooltip" className="pointer-events-none absolute bottom-[calc(100%+8px)] left-1/2 z-20 w-64 -translate-x-1/2 rounded-md border border-black/10 bg-[#fffdfa] px-3 py-2 text-left text-xs font-normal leading-5 text-[#263b33] opacity-0 shadow-lg transition-opacity group-hover:opacity-100 group-focus-within:opacity-100">Verified within the last 30 days using an authenticator app or another approved method. This helps show the account is actively controlled by a person; it is not a guarantee about the person or their activity.</span>
                </>}
              />
            </div>}
            <div className="mt-5 w-full [&>button]:min-h-11 [&>button]:w-full [&>button]:rounded-md">
              {isOwn ? <Link href="/app/profile/setup" className="btn-primary inline-flex min-h-11 w-full items-center justify-center rounded-md text-center">Edit profile</Link> : <IcebreakerModal action={startConversation} userId={targetId} username={username} recipientName={displayName} interestNames={interests.flatMap((interest) => { const name = interestName(interest.interests); return name ? [name] : []; })} />}
            </div>
            {!isOwn && <div className="mt-3 w-full"><details className="relative"><summary className="flex cursor-pointer list-none items-center justify-between rounded-md border border-[#d7d0c3] bg-[#fbfaf6] px-4 py-3 text-sm font-medium text-[#33443e] transition hover:bg-white/75">More actions <span className="text-black/40" aria-hidden="true">⌄</span></summary><div className="absolute left-0 top-[calc(100%+6px)] z-20 w-full rounded-md border border-black/10 bg-[#fffdfa] p-2 shadow-lg"><div className="p-1"><BlockControl blocked={blocked} id={targetId} username={username} /></div><div className="border-t border-black/[0.06] p-1 pt-2">{reportControl}</div></div></details></div>}
          </aside>

          <div className="min-w-0 pt-1 lg:pt-8">
            <h1 className="font-serif text-[clamp(3.1rem,3.6vw,3.75rem)] leading-[.96] tracking-[-0.045em] text-[#10231d]">{displayName}{age !== null ? `, ${age}` : ""}</h1>
            ...
            <div className="mt-5 flex min-h-5 flex-wrap items-center gap-x-4 gap-y-2 text-sm text-black/60">
              ...
              {(profile.role === "admin" || profile.role === "moderator") && <>
                ...
                <span className="inline-flex items-center gap-1 font-medium text-black/60">
                  ...
                  {profile.role === "admin" && username.toLowerCase() === "admin" && <span className="group relative inline-flex items-center">
                    <button type="button" aria-label="About the site owner and developer" aria-describedby="site-owner-tooltip" className="inline-flex h-5 w-5 items-center justify-center rounded-full text-[13px] leading-none text-black/60 outline-none transition hover:text-[#075d46] focus-visible:ring-2 focus-visible:ring-[#087456]/40"><span aria-hidden="true">♛</span></button>
                    <span id="site-owner-tooltip" role="tooltip" className="pointer-events-none absolute bottom-[calc(100%+8px)] left-1/2 z-20 w-56 -translate-x-1/2 rounded-md border border-black/10 bg-[#fffdfa] px-3 py-2 text-left text-xs font-normal leading-5 text-[#263b33] opacity-0 shadow-lg transition-opacity group-hover:opacity-100 group-focus-within:opacity-100">This is the site owner and developer. Approach at your own risk.</span>
                  </span>}
                </span>
              </>}
            </div>
            {profile.activity_rank && <div className="mt-5 text-sm text-black/50"><p className="font-semibold text-[#075d46]">{profile.activity_rank}</p>{profile.activity_rank_flavor && <p className="mt-1 italic">{profile.activity_rank_flavor}</p>}</div>}
          ...
          <aside className="order-4 border-t border-black/10 pt-8 lg:col-span-2 xl:order-none xl:col-span-1 xl:row-span-2 xl:border-l xl:border-t-0 xl:pl-10 xl:pt-1 2xl:pl-12">
            ...
            {languageCompatibility && (languageCompatibility.sharedLanguages.length > 0 || languageCompatibility.exchangeLanguages.length > 0) && <div className="mt-4 border-t border-black/[0.08] pt-4 text-sm leading-6 text-[#075d46]" aria-label="Language compatibility">
              {languageCompatibility.sharedLanguages.length > 0 && <p><span className="font-medium">You both speak:</span> {languageCompatibility.sharedLanguages.join(", ")}</p>}
              {languageCompatibility.exchangeLanguages.length > 0 && <p className={languageCompatibility.sharedLanguages.length > 0 ? "mt-1 text-black/55" : "text-[#075d46]"}><span className="font-medium">Language exchange:</span> {languageCompatibility.exchangeLanguages.join(", ")}</p>}
            </div>}
            <InterestsSection interests={interests} />
            <FriendshipDestinationsSection destinations={friendshipDestinations} />
            {responseCopy && <section aria-labelledby="response-heading" className="mt-9 border-t border-black/10 pt-7"><DetailHeading icon="response"><span id="response-heading">Response rate</span></DetailHeading><p className="mt-5 text-[15px] leading-6 text-black/70">{responseCopy}</p></section>}
            {communicationModes.length > 0 && <section aria-labelledby="communication-heading" aria-label={communicationPreferenceSummary ?? undefined} className="mt-9 border-t border-black/10 pt-7"><DetailHeading icon="communication"><span id="communication-heading">Communication preferences</span></DetailHeading><div className="mt-4 space-y-3">{communicationModes.map((mode) => <p key={mode} className="flex items-center gap-3 text-[15px] text-black/70"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#e8ede5] text-[#075d46]"><DetailIcon name="communication" /></span>{mode}</p>)}</div></section>}
          </aside>

          <AboutSection bio={profile.bio} />
          <PersonalityLifestyleSection personality={personality} />
        </section>
      </div>
    </main>
  );
