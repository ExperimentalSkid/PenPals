...
function StatCard({ label, value, note }: { label: string; value: string; note?: string }) {
  return <div className="border border-black/10 bg-white/35 px-5 py-5"><p className="text-xs font-semibold uppercase tracking-[.12em] text-black/45">{label}</p><p className="mt-3 font-serif text-3xl text-primary">{value}</p>{note && <p className="mt-2 text-xs text-black/45">{note}</p>}</div>;
}
...
export default async function StaffAnalytics({ searchParams }: { searchParams: Promise<{ range?: string; from?: string; to?: string }> }) {
  ...
  return <AdminPage>
    <AdminHeader active="analytics" eyebrow="Live operations · Read only" title="Analytics" description="View platform activity, engagement, and moderation work. Private content is not exposed." backHref={role === "admin" ? "/app/admin" : "/app/moderation"} backLabel={role === "admin" ? "Admin Center" : "Moderator Panel"} isAdmin={role === "admin"} accessLabel={role === "admin" ? "Admin Center" : "Moderator Panel"} />
    ...
    {warning && <p className="notice notice-warning mt-5">{warning}</p>}
    {error ? <p role="alert" className="notice notice-error mt-8">Analytics could not be loaded. Refresh before relying on these figures.</p> : <>
