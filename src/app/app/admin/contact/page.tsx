...
export default async function ContactInbox({ searchParams }: { searchParams: Promise<{ status?: string; category?: string; assignment?: string; q?: string; sort?: string; page?: string; error?: string }> }) {
  ...
  return <AdminPage>
    <AdminHeader active="contact" eyebrow="Public contact queue" title="Contact Inbox" description="Review messages from visitors who contacted pen-pals.net before signing in." isAdmin={role === "admin"} />
    ...
    {error && <p role="alert" className="mt-5 border-l-2 border-red-400 px-3 py-2 text-sm text-red-700">Contact messages could not be loaded. Refresh before relying on this queue.</p>}
    <section className="admin-section mt-8" aria-label="Contact messages">
      <div className="admin-table overflow-x-auto">
        <div className="min-w-[1060px]">
          ...
          {tickets.length ? tickets.map((ticket: any) => <Link key={ticket.id} href={`/app/admin/support/${ticket.id}?return_to=${encodeURIComponent(`/app/admin/contact?${queryFor(page)}`)}`} className="admin-row block hover:text-[#075d46]"><div className="grid grid-cols-[110px_minmax(0,1.3fr)_minmax(0,1fr)_150px_130px_120px_145px_145px] items-center gap-3"><div className="font-mono text-xs font-semibold text-[#10231d]">{ticket.ticket_code}</div><div className="min-w-0"><p className="truncate font-medium text-[#10231d]">{ticket.subject}</p><p className="mt-1 text-xs text-black/45">{labelFor(ticket.ticket_type)}</p></div><div className="min-w-0"><p className="truncate text-sm">{ticket.requester_display_name || "Public contact"}</p></div><div><StatusChip value={labelFor(ticket.category)} tone="neutral" /></div><div><StatusChip value={labelFor(ticket.status)} tone={toneForStatus(ticket.status)} /></div><div><StatusChip value={labelFor(ticket.priority)} tone={priorityTone(ticket.priority)} /></div><div className="truncate text-xs text-black/50">{ticket.assigned_staff_name || "Unassigned"}</div><div className="text-xs text-black/50">{dateLabel(ticket.updated_at)}</div></div></Link>) : <div className="px-4 py-12 text-sm text-black/50"><p>{error ? "No contact message data is available." : "No contact messages match these filters."}</p><p className="mt-2 text-xs text-black/40">New public contact messages will appear here as they arrive.</p></div>}
        </div>
      </div>
    </section>
    <nav className="mt-8 flex flex-wrap items-center justify-between gap-4 border-t border-black/10 pt-5 text-sm" aria-label="Contact inbox pagination"><span className="text-black/50">{total ? `Showing ${(page - 1) * PAGE_SIZE + 1}–${Math.min(page * PAGE_SIZE, total)} of ${total} messages` : "0 messages"}</span><div className="flex items-center gap-2"><span className="sr-only">Page navigation</span>{page > 1 ? <Link href={`/app/admin/contact?${queryFor(page - 1)}`} aria-label="Previous page" className="rounded-md border border-black/10 px-3 py-2 text-[#087456] hover:bg-white/70">←</Link> : <span aria-disabled="true" aria-label="Previous page" className="rounded-md border border-black/10 px-3 py-2 text-black/25">←</span>}<span aria-current="page" className="rounded-md bg-[#075d46] px-3 py-2 font-semibold text-white">{page}</span>{page < pageCount ? <Link href={`/app/admin/contact?${queryFor(page + 1)}`} aria-label="Next page" className="rounded-md border border-black/10 px-3 py-2 text-[#087456] hover:bg-white/70">→</Link> : <span aria-disabled="true" aria-label="Next page" className="rounded-md border border-black/10 px-3 py-2 text-black/25">→</span>}<span className="ml-3 rounded-md border border-black/10 px-3 py-2 text-black/55">25 per page</span></div></nav>
  </AdminPage>;
}
