...
export default async function SupportInbox({ searchParams }: { searchParams: Promise<{ status?: string; category?: string; assignment?: string; q?: string; sort?: string; page?: string; error?: string }> }) {
  ...
  return <AdminPage>
    <AdminHeader active="support" eyebrow="Support queue" title="Support Inbox" description="Review user support requests. Open each ticket to see the conversation." isAdmin={role === "admin"} />
    ...
    {error && <p role="alert" className="mt-5 border-l-2 border-red-400 px-3 py-2 text-sm text-red-700">Support tickets could not be loaded. Refresh before relying on this queue.</p>}
    ...
    <nav className="mt-8 flex flex-wrap items-center justify-between gap-4 border-t border-black/10 pt-5 text-sm" aria-label="Support ticket pagination"><span className="text-black/50">{total ? `Showing ${(page - 1) * PAGE_SIZE + 1}–${Math.min(page * PAGE_SIZE, total)} of ${total} tickets` : "0 tickets"}</span><div className="flex items-center gap-2"><span className="sr-only">Page navigation</span>{page > 1 ? <Link href={`/app/admin/support?${queryFor(page - 1)}`} aria-label="Previous page" className="rounded-md border border-black/10 px-3 py-2 text-[#087456] hover:bg-white/70">←</Link> : <span aria-disabled="true" aria-label="Previous page" className="rounded-md border border-black/10 px-3 py-2 text-black/25">←</span>}<span aria-current="page" className="rounded-md bg-[#075d46] px-3 py-2 font-semibold text-white">{page}</span>{page < pageCount ? <Link href={`/app/admin/support?${queryFor(page + 1)}`} aria-label="Next page" className="rounded-md border border-black/10 px-3 py-2 text-[#087456] hover:bg-white/70">→</Link> : <span aria-disabled="true" aria-label="Next page" className="rounded-md border border-black/10 px-3 py-2 text-black/25">→</span>}<span className="ml-3 rounded-md border border-black/10 px-3 py-2 text-black/55">25 per page</span></div></nav>
  </AdminPage>;
}
