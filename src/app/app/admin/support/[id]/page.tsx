      ...
      <div>
        <section aria-labelledby="ticket-details-heading">
          ...
          <dl className="mt-4 grid gap-x-8 gap-y-4 border-y border-black/10 py-5 text-sm sm:grid-cols-2">
            ...
            <div><dt className="text-black/45">{isPublicContact ? "Contact" : "Requester"}</dt><dd className="mt-1">{isPublicContact ? <span>{ticket.requester?.display_name || "Public contact"}{ticket.contact?.email ? <span className="mt-1 block text-xs text-black/50">{ticket.contact.email}</span> : null}</span> : ticket.requester ? (role === "admin" && ticket.requester.id ? <Link href={`/app/admin/users/${ticket.requester.id}`} className="text-[#087456] hover:underline">{ticket.requester.display_name || ticket.requester.username}</Link> : <span>{ticket.requester.display_name || ticket.requester.username}</span>) : "Former account"}</dd></div>
        ...
        <section className="mt-10" aria-labelledby="ticket-conversation-heading">
          <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="admin-eyebrow">Conversation</p><h2 id="ticket-conversation-heading" className="mt-1 font-serif text-2xl text-[#10231d]">{isPublicContact ? "Contact messages" : "Support messages"}</h2></div><p className="text-sm text-black/45">{messages.length} message{messages.length === 1 ? "" : "s"}</p></div>
        ...
        <div className="mt-10 grid gap-8 xl:grid-cols-2">
          <section className="border border-[#087456]/20 bg-white/30 p-5" aria-labelledby="public-reply-heading">
            ...
            <h2 id="public-reply-heading" className="mt-1 font-serif text-2xl text-[#10231d]">{isPublicContact ? "Reply by email" : "Public reply"}</h2>
