...
function statusLabel(value: unknown) {
  return ({ open: "Open", waiting_staff: "Waiting for staff", waiting_user: "Waiting for user", resolved: "Resolved" } as Record<string, string>)[String(value)] ?? labelFor(value);
}
...
export default async function SupportTicket({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ return_to?: string; error?: string; updated?: string }> }) {
  ...
  const returnTo = safeAdminReturnTo(query.return_to) ?? "/app/admin/support";
  ...
  return <AdminPage>
    <AdminHeader active="support" eyebrow="Support ticket" title={ticket.ticket_code} description={ticket.subject} backHref={returnTo} backLabel="Support Inbox" isAdmin={role === "admin"}>
      <div className="flex flex-wrap justify-end gap-2"><StatusChip value={statusLabel(status)} tone={toneForStatus(status)} /><StatusChip value={labelFor(ticket.priority)} tone={priorityTone(ticket.priority)} /></div>
    </AdminHeader>
    {query.error && <p role="alert" className="mt-6 border-l-2 border-red-400 px-3 py-2 text-sm text-red-700">{query.error}</p>}
    {updatedMessage && <p role="status" className="mt-6 border-l-2 border-[#087456] px-3 py-2 text-sm text-[#075d46]">{updatedMessage}</p>}

    <section className="mt-8 grid gap-10 lg:grid-cols-[minmax(0,1fr)_300px]">
      <div>
        <section aria-labelledby="ticket-details-heading">
          <h2 id="ticket-details-heading" className="font-serif text-2xl text-[#10231d]">Ticket details</h2>
          <dl className="mt-4 grid gap-x-8 gap-y-4 border-y border-black/10 py-5 text-sm sm:grid-cols-2">
            ...
            <div><dt className="text-black/45">Requester</dt><dd className="mt-1">{ticket.requester ? (role === "admin" ? <Link href={`/app/admin/users/${ticket.requester.id}`} className="text-[#087456] hover:underline">{ticket.requester.display_name || ticket.requester.username}</Link> : <span>{ticket.requester.display_name || ticket.requester.username}</span>) : "Former account"}</dd></div>
        ...
        <section className="mt-10" aria-labelledby="ticket-conversation-heading">
          <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="admin-eyebrow">Conversation</p><h2 id="ticket-conversation-heading" className="mt-1 font-serif text-2xl text-[#10231d]">Support messages</h2></div><p className="text-sm text-black/45">{messages.length} message{messages.length === 1 ? "" : "s"}</p></div>
          {messages.length ? <div className="mt-4 divide-y divide-black/10 border-y border-black/10">{messages.map((message: any, index: number) => <article key={message.id} className={`py-5 ${message.is_internal ? "bg-[#fff9ec] px-4" : ""}`}><div className="flex flex-wrap items-baseline justify-between gap-3"><div className="flex flex-wrap items-center gap-2"><p className="font-medium">{message.author_name}</p><StatusChip value={message.is_internal ? "Internal note · staff only" : "Public reply · requester can see"} tone={message.is_internal ? "warn" : "good"} /></div><span className="text-xs text-black/45">{dateLabel(message.created_at)}</span></div><p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-black/70">{message.body}</p>{index === 0 && attachments.length > 0 && <SupportAttachmentViewer attachments={attachments} />}</article>)}</div> : <>{<p className="mt-4 border-y border-black/10 py-10 text-sm text-black/50">No messages are attached to this ticket yet.</p>}{attachments.length > 0 && <SupportAttachmentViewer attachments={attachments} />}</>}
        </section>

        <div className="mt-10 grid gap-8 xl:grid-cols-2">
          <section className="border border-[#087456]/20 bg-white/30 p-5" aria-labelledby="public-reply-heading">
            <p className="admin-eyebrow">Requester-visible</p>
            <h2 id="public-reply-heading" className="mt-1 font-serif text-2xl text-[#10231d]">Public reply</h2>
            <p className="mt-2 text-sm text-black/55">This message is sent to the requester and moves the ticket to <strong>Waiting for user</strong>.</p>
            {status === "resolved" ? <p className="mt-5 text-sm text-black/50">Reopen the ticket before sending a public reply.</p> : canAct ? <form action={staffReplyToSupportTicket} className="mt-5 space-y-3"><input type="hidden" name="ticket_id" value={id} /><input type="hidden" name="return_to" value={returnTo} /><input type="hidden" name="submission_token" value={crypto.randomUUID()} /><label htmlFor="support-public-reply" className="block text-sm font-medium text-[#263b33]">Message to requester<textarea id="support-public-reply" name="body" required maxLength={4000} rows={5} className="field mt-2 min-h-28 w-full resize-y" placeholder="Write the update the requester should see." /></label><div className="flex justify-end"><StaffSupportSubmitButton label="Send public reply" pendingLabel="Sending…" primary /></div></form> : <p className="mt-5 text-sm text-black/50">Claim this ticket before sending a public reply.</p>}
          </section>
          <section className="border border-[#c8871b]/30 bg-[#fff9ec] p-5" aria-labelledby="internal-note-heading">
            ...
            <h2 id="internal-note-heading" className="mt-1 font-serif text-2xl text-[#10231d]">Internal staff note</h2>
      ...
      <aside className="space-y-8 border-l border-black/10 pl-8">
        <section aria-labelledby="queue-context-heading"><h2 id="queue-context-heading" className="font-serif text-2xl text-[#10231d]">Queue context</h2><p className="mt-3 text-sm text-black/55">This ticket is part of the Support Inbox and is separate from moderation cases.</p><dl className="mt-5 space-y-3 border-t border-black/10 pt-4 text-sm"><div><dt className="text-black/45">Ticket type</dt><dd className="mt-1">{ticket.ticket_type}</dd></div><div><dt className="text-black/45">Status</dt><dd className="mt-1">{statusLabel(status)}</dd></div><div><dt className="text-black/45">Priority</dt><dd className="mt-1">{labelFor(ticket.priority)}</dd></div></dl></section>
        <section className="border-t border-black/10 pt-7" aria-labelledby="assignment-heading"><h2 id="assignment-heading" className="font-serif text-xl text-[#10231d]">Assignment</h2><p className="mt-3 text-sm text-black/55">{assignedStaffId ? `Assigned to ${ticket.assigned_staff?.display_name || "another staff member"}.` : "No staff member owns this ticket yet."}</p>{!assignedStaffId ? <form action={claimSupportTicket} className="mt-4"><input type="hidden" name="ticket_id" value={id} /><input type="hidden" name="return_to" value={returnTo} /><StaffSupportSubmitButton label="Claim ticket" pendingLabel="Claiming…" primary /></form> : (assignedStaffId === uid || role === "admin") ? <form action={releaseSupportTicket} className="mt-4"><input type="hidden" name="ticket_id" value={id} /><input type="hidden" name="return_to" value={returnTo} /><StaffSupportSubmitButton label="Release assignment" pendingLabel="Releasing…" /></form> : <p className="mt-4 text-xs text-black/45">Only the assigned staff member or an administrator can change this assignment.</p>}</section>
        <section className="border-t border-black/10 pt-7" aria-labelledby="status-heading"><h2 id="status-heading" className="font-serif text-xl text-[#10231d]">Ticket status</h2>{canAct ? <form action={setSupportTicketStatus} className="mt-4 space-y-3"><input type="hidden" name="ticket_id" value={id} /><input type="hidden" name="return_to" value={returnTo} /><label className="block text-sm text-black/60">Set status<select name="status" defaultValue={status} className="field mt-2 w-full"><option value="open">Open</option><option value="waiting_staff">Waiting for staff</option><option value="waiting_user">Waiting for user</option><option value="resolved">Resolved</option></select></label><StaffSupportSubmitButton label={status === "resolved" ? "Reopen or update status" : "Save status"} pendingLabel="Saving…" primary /></form> : <p className="mt-4 text-sm text-black/55">Claim this ticket before changing its status.</p>}</section>
      </aside>
    </section>
  </AdminPage>;
}
