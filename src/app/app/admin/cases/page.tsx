...
export default async function AdminCases({ searchParams }: { searchParams: Promise<{ status?: string; page?: string; error?: string; return_to?: string }> }) {
  ...
  return <AdminPage>
    <AdminHeader active="cases" eyebrow="Trust & safety · Moderation" title="Cases" description="Review moderation cases, related reports, ownership, and history." backHref={returnTo ?? "/app/admin"} backLabel={returnTo ? "Back to investigation" : "Admin Center"} isAdmin={role === "admin"} />
    <form className="admin-toolbar flex flex-wrap items-end gap-3"><label className="text-sm text-black/60">Status<select name="status" defaultValue={status ?? ""} className="field mt-2 block w-auto"><option value="">All cases</option>{statuses.map((item) => <option key={item} value={item}>{labelFor(item)}</option>)}</select></label><button className="btn-primary px-4 py-2.5 text-sm">Apply case filter</button></form>
    {filters.error && <p role="alert" className="mt-5 border-l-2 border-red-400 px-3 py-2 text-sm text-red-700">{filters.error}</p>}{error && <p role="alert" className="mt-5 border-l-2 border-red-400 px-3 py-2 text-sm text-red-700">The case queue could not be loaded.</p>}
    <section className="admin-section mt-8" aria-label="Moderation cases">{cases.length ? <div className="admin-table"><div className="admin-table-head md:grid md:grid-cols-[minmax(0,1fr)_140px_170px]"><span>Case / target</span><span>Priority &amp; source</span><span>Owner · updated</span></div>{cases.map((item: any) => <Link key={item.id} href={withAdminReturnTo(`/app/admin/cases/${item.id}`, queueContext)} className="admin-row"><div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_140px_170px] md:items-center"><div><div className="flex flex-wrap items-center gap-3"><p className="font-serif text-xl text-[#10231d]">{item.subject_display_name || item.subject_username || "Unidentified account"}</p><StatusChip value={item.status} tone={toneForStatus(item.status)} />{item.needs_admin_review && item.status !== "resolved" && item.status !== "dismissed" && <span className="admin-status admin-status-warn">Admin attention</span>}</div><p className="mt-1 text-sm text-black/60">{item.primary_target_type ?? "Report"} · {item.report_count} report{item.report_count === 1 ? "" : "s"} · {item.independent_reporter_count} independent</p></div><div className="text-sm"><p className="font-semibold text-[#10231d]">Priority {item.priority}</p><p className="mt-1 text-xs text-black/45">{item.source === "automated_flag" ? `${item.automated_flag_count ?? 0} automated flag${item.automated_flag_count === 1 ? "" : "s"}` : item.source === "mixed" ? "Human + automated" : "Human report"}</p></div><div className="text-xs text-black/50"><p>{item.assigned_staff_name ? `Owned by ${item.assigned_staff_name}` : "Unassigned"}</p><p className="mt-1">{new Date(item.updated_at).toLocaleString()}</p></div></div></Link>)}</div> : <p className="border-t border-black/10 py-10 text-sm text-black/50">No cases match these filters.</p>}</section>
