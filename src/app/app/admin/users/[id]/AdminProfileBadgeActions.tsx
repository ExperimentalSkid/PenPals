...
export default function AdminProfileBadgeActions({ userId, badges }: { userId: string; badges: BadgeRecord[] }) {
  ...
  return <section aria-labelledby="profile-badges-heading" className="border-t border-black/10 pt-6">
    <h2 id="profile-badges-heading" className="font-serif text-2xl text-[#10231d]">Profile badges</h2>
    <p className="mt-3 text-sm leading-6 text-black/55">Assign or remove community badges here. Every change requires a reason and is recorded in the audit log.</p>
...
function BadgeFields({ assigned }: { assigned: Set<string> }) {
  ...
  return <>
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="text-sm font-medium text-[#263b33]">Badge
        ...
        <label className="text-sm font-medium text-[#263b33]">Action
      ...
      <label className="block text-sm font-medium text-[#263b33]">Reason <span className="text-xs font-normal text-black/45">(required)</span>
