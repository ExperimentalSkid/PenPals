...
export default async function AdminReports({ searchParams }: { searchParams: Promise<{ report?: string; status?: string; reason?: string; target_type?: string; page?: string; error?: string; updated?: string; return_to?: string }> }) {
  ...
  return <AdminPage><AdminHeader active="reports" eyebrow="Trust & safety · Review queue" title="Reports" description="Review reported profiles and conversations with their context." backHref={returnTo ?? "/app/admin"} backLabel={returnTo ? "Back to investigation" : "Admin Center"} isAdmin={role === "admin"} />
    <form className="admin-toolbar flex flex-wrap items-end gap-3"><label className="text-sm text-black/60">Status<select name="status" defaultValue={filters.status ?? ""} className="field mt-2 block w-auto"><option value="">All statuses</option>{statuses.map((status) => <option key={status} value={status}>{labelFor(status)}</option>)}</select></label><label className="text-sm text-black/60">Reason<select name="reason" defaultValue={filters.reason ?? ""} className="field mt-2 block w-auto"><option value="">All reasons</option>{reasons.map((reason) => <option key={reason} value={reason}>{labelFor(reason)}</option>)}</select></label><label className="text-sm text-black/60">Target<select name="target_type" defaultValue={filters.target_type ?? ""} className="field mt-2 block w-auto"><option value="">All targets</option><option value="profile">Profile</option><option value="introduction">Introduction</option><option value="message">Message</option></select></label><button className="btn-primary px-4 py-2.5 text-sm">Apply report filters</button></form>
    {filters.error && <p role="alert" className="mt-5 border-l-2 border-red-400 px-3 py-2 text-sm text-red-700">{filters.error}</p>}{filters.updated && <p role="status" className="mt-5 border-l-2 border-[#087456] px-3 py-2 text-sm text-[#075d46]">Report status updated.</p>}{reportsError && <p role="alert" className="mt-5 border-l-2 border-red-400 px-3 py-2 text-sm text-red-700">Reports could not be loaded. Refresh before relying on this queue.</p>}{peopleError && <p role="alert" className="mt-5 border-l-2 border-red-400 px-3 py-2 text-sm text-red-700">Reporter context could not be loaded. Refresh before relying on reporter names.</p>}{detailError && <p role="alert" className="mt-5 border-l-2 border-red-400 px-3 py-2 text-sm text-red-700">Report details could not be loaded.</p>}{auditError && <p role="alert" className="mt-5 border-l-2 border-red-400 px-3 py-2 text-sm text-red-700">Report audit history could not be loaded.</p>}{targetUserDetailError && <p role="alert" className="mt-5 border-l-2 border-red-400 px-3 py-2 text-sm text-red-700">Related account context could not be loaded.</p>}
    <div className="mt-8 grid gap-10 lg:grid-cols-[minmax(0,.9fr)_minmax(0,1.1fr)]"><section aria-label="Reports">{reportsError ? <p className="border-t border-black/10 py-10 text-sm text-red-700">Reports could not be loaded.</p> : (reports ?? []).length ? <div className="admin-table">{reports.map((item: any) => <Link key={item.id} href={`/app/admin/reports?report=${item.id}${filters.status ? `&status=${encodeURIComponent(filters.status)}` : ""}&page=${page}`} className={`admin-row block ${filters.report === item.id ? "bg-[#e9eee8]" : ""}`}><div className="flex items-center justify-between gap-3"><p className="font-semibold capitalize">{item.target_type} <span className="font-normal text-black/45">· {labelFor(item.reason)}</span></p><StatusChip value={labelFor(item.status)} tone={toneForStatus(item.status)} /></div><p className="mt-2 text-xs text-black/45">{people.get(item.reporter_id)?.display_name ?? "Restricted reporter"} · {new Date(item.created_at).toLocaleString()}</p>{item.details && <p className="mt-2 line-clamp-2 text-sm text-black/55">{item.details}</p>}</Link>)}</div> : <p className="border-t border-black/10 py-10 text-sm text-black/50">No reports match these filters.</p>}<nav className="mt-6 flex items-center justify-between border-t border-black/10 pt-4 text-sm"><span className="text-black/50">Page {page} of {pageCount}</span><div className="flex gap-4">{page > 1 && <Link href={listLink(page - 1)} className="text-[#087456] hover:underline">Previous</Link>}{page < pageCount && <Link href={listLink(page + 1)} className="text-[#087456] hover:underline">Next</Link>}</div></nav></section>
      {detail && <section className="border-l border-black/10 pl-8"><h2 className="font-serif text-2xl text-[#10231d]">Report detail</h2><dl className="mt-5 grid gap-2 border-y border-black/10 py-4 text-sm"><div className="flex justify-between gap-4"><dt className="text-black/45">Reporter</dt><dd>{people.get(report?.reporter_id)?.display_name ?? "Restricted reporter"}</dd></div><div className="flex justify-between gap-4"><dt className="text-black/45">Target</dt><dd>{targetType ?? "Unknown"}</dd></div><div className="flex justify-between gap-4"><dt className="text-black/45">Reason</dt><dd>{report?.reason ?? "—"}</dd></div><div className="flex justify-between gap-4"><dt className="text-black/45">Status</dt><dd>{report?.status ?? "—"}</dd></div></dl>{report?.case_id && <div className="mt-5 text-sm"><span className="text-black/45">Moderation case</span>{" "}<Link href={withAdminReturnTo(`/app/admin/cases/${report.case_id}`, reportContext)} className="text-[#087456] underline">Open case →</Link></div>}{targetType === "profile" && <div className="mt-6"><h3 className="font-serif text-xl">Reported profile</h3>{target?.username && <Link href={withAdminReturnTo(`/app/profile/${target.username}?from=admin`, reportContext)} className="mt-3 inline-block text-[#087456] underline">{target.display_name ?? target.username} (@{target.username})</Link>}{target?.bio && <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-black/70">{target.bio}</p>}{target?.quote && <p className="mt-3 font-serif text-lg text-[#075d46]">“{target.quote}”</p>}</div>}{targetType === "introduction" && <div className="mt-6"><h3 className="font-serif text-xl">Reported introduction</h3><p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-black/70">{target?.icebreaker ?? target?.body ?? "No preserved text."}</p><p className="mt-3 text-xs text-black/45">Status: {target?.status ?? "—"}</p></div>}{targetType === "message" && <div className="mt-6"><h3 className="font-serif text-xl">Reported message</h3><p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-black/70">{target?.body ?? "No preserved text."}</p><p className="mt-3 text-xs text-black/45">Sent {target?.created_at ? new Date(target.created_at).toLocaleString() : "—"}</p></div>}{conversationId && <div className="mt-6 border-t border-black/10 pt-5"><h3 className="font-serif text-xl">Related conversation</h3><Link href={withAdminReturnTo(`/app/admin/conversations/${conversationId}?report=${report?.id}`, reportContext)} className="mt-2 inline-block text-sm text-[#087456] hover:underline">Open privileged review →</Link></div>}{targetUserDetail?.profile?.username && <div className="mt-6 border-t border-black/10 pt-5"><h3 className="font-serif text-xl">Related account</h3><Link href={withAdminReturnTo(`/app/admin/users/${targetUserId}`, reportContext)} className="mt-2 inline-block text-sm text-[#087456] hover:underline">{targetUserDetail.profile.display_name ?? targetUserDetail.profile.username} · View user</Link><p className="mt-2 text-xs text-black/45">{relatedReports.length} report{relatedReports.length === 1 ? "" : "s"} involving this account · {relatedAudit.length} audit entr{relatedAudit.length === 1 ? "y" : "ies"}</p>{relatedReports.length > 1 && <div className="mt-3 border-t border-black/10 pt-3"><p className="text-xs uppercase tracking-[.1em] text-black/40">Previous reports</p>{relatedReports.filter((item: any) => item.id !== report?.id).slice(0, 3).map((item: any) => <Link key={item.id} href={withAdminReturnTo(`/app/admin/reports?report=${item.id}`, reportContext)} className="mt-2 block text-xs text-[#087456] hover:underline">{item.reason} · {item.status}</Link>)}</div>}{relatedAudit.length > 0 && <div className="mt-3 border-t border-black/10 pt-3"><p className="text-xs uppercase tracking-[.1em] text-black/40">Recent actions</p>{relatedAudit.slice(0, 3).map((entry: any) => <p key={entry.id} className="mt-2 text-xs text-black/55">{entry.action.replaceAll("_", " ")} · {new Date(entry.created_at).toLocaleString()}</p>)}</div>}</div>}<form action={updateAdminReportStatus} className="mt-8 flex flex-wrap items-center gap-3 border-t border-black/10 pt-6"><input type="hidden" name="report_id" value={filters.report}/><label htmlFor="new-report-status" className="sr-only">New status</label><select id="new-report-status" name="status" defaultValue={report?.status === "open" ? "reviewing" : report?.status ?? "reviewing"} className="field w-auto"><option>reviewing</option><option>actioned</option><option>dismissed</option></select><button className="btn-primary px-4 py-2.5 text-sm">Update status</button></form><h3 className="mt-8 border-t border-black/10 pt-6 font-serif text-xl">Audit history</h3><div className="mt-4 space-y-3">{auditError ? <p className="text-sm text-red-700">Audit history could not be loaded.</p> : audit.length ? audit.map((entry: any) => <p key={entry.id} className="text-xs leading-5 text-black/55">{entry.action.replaceAll("_", " ")} · {entry.old_status ?? "—"} → {entry.new_status ?? "—"}<span className="block text-black/35">{new Date(entry.created_at).toLocaleString()}</span></p>) : <p className="text-sm text-black/45">No audit entries yet.</p>}</div></section>}
    </div></AdminPage>;
}
