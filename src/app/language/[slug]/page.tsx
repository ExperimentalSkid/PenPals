...
export async function generateMetadata({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }): Promise<Metadata> {
  ...
  const [surface, locale] = await Promise.all([loadPublicSeoSurface("language", slug), resolveLocale()]);
  return seoSurfaceMetadata("language", surface, slug, Object.keys(query).length > 0, locale);
}
...
export default async function LanguageSeoPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (!surface) notFound();
  const query = await searchParams;
  if (slug !== surface.canonical_slug || Object.keys(query).length > 0) permanentRedirect(seoSurfacePath("language", surface.canonical_slug, locale));
