-- Apply record-level retention precisely.  A held report/evidence/audit row
-- is retained, while unrelated rows for the same account are still erased.
...
begin
  ...
  select u.last_sign_in_at into recent_sign_in
    from auth.users u where u.id = me for update;
  ...
  for conversation_id in
    select distinct cp.conversation_id from public.conversation_participants cp where cp.user_id = me
  loop
    perform 1 from public.conversations c where c.id = conversation_id for update;
  end loop;

  select coalesce(array_agg(r.id), array[]::uuid[]) into report_ids
  ...
  select o.name from storage.objects o
   where o.bucket_id = 'avatars' and (storage.foldername(o.name))[1] = me::text
  on conflict (path) do nothing;

  retain_all_evidence := public.retention_policy_enabled('moderation_evidence')
    or exists (select 1 from public.data_retention_holds h
       where h.category = 'moderation_evidence' and h.record_id is null
         and h.released_at is null and h.started_at <= now());
  retain_all_audit := public.retention_policy_enabled('moderation_audit')
    or exists (select 1 from public.data_retention_holds h
       where h.category = 'moderation_audit' and h.record_id is null
         and h.released_at is null and h.started_at <= now());

  -- Retain only reports covered by the configured policy or an active case
  -- hold.  Reporter-owned reports are always erased with the account.
  update public.reports r
     set target_id = null, target_profile_id = null,
         target_introduction_id = null, target_message_id = null,
   ...
   where r.id = any(report_ids)
     and (retain_all_evidence or exists (
       select 1 from public.data_retention_holds h
        where h.category = 'moderation_evidence' and h.record_id = r.id
          and h.released_at is null and h.started_at <= now()
     ));
  delete from public.reports r
   where r.reporter_id = me
      or (r.id = any(report_ids) and not (retain_all_evidence or exists (
        select 1 from public.data_retention_holds h
         where h.category = 'moderation_evidence' and h.record_id = r.id
           and h.released_at is null and h.started_at <= now()
      )));

  update public.profile_moderation_evidence e
     set target_user_id = null, previous_value = null,
         reason = 'Redacted after account deletion'
   where e.target_user_id = me
     and (retain_all_evidence or exists (
       select 1 from public.data_retention_holds h
        where h.category = 'moderation_evidence' and h.record_id = e.id
          and h.released_at is null and h.started_at <= now()
     ));
  delete from public.profile_moderation_evidence e
   where e.target_user_id = me
      and not (retain_all_evidence or exists (
        select 1 from public.data_retention_holds h
         where h.category = 'moderation_evidence' and h.record_id = e.id
           and h.released_at is null and h.started_at <= now()
      ));

  update public.moderation_audit_log a
     set moderator_id = null, target_user_id = null,
   ...
   where (a.moderator_id = me or a.target_user_id = me or a.report_id = any(report_ids))
     and (retain_all_audit or exists (
       select 1 from public.data_retention_holds h
        where h.category = 'moderation_audit' and h.record_id = a.id
          and h.released_at is null and h.started_at <= now()
     ));
  ...
  delete from public.conversations c where not exists (
