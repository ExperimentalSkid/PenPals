-- Preserve the prior status in the audit record while retaining the active
-- claim requirement added for moderator case mutations.
...
begin
  ...
  select status into previous_status
   ...
   where id = case_uuid
   for update;
  if previous_status is null then raise exception 'Case not found'; end if;
  ...
  insert into public.moderation_audit_log(moderator_id, case_id, target_user_id, action, old_status, new_status, metadata)
    values (auth.uid(), case_uuid, updated_case.subject_user_id, 'case_status_change', previous_status, new_status,
            jsonb_build_object('reason', clean_reason, 'resolution_category', resolution, 'assignment_required', not public.is_admin()));
  return updated_case;
end;
$$;
