-- Apply the scoped-hold deletion function after the previous migration has
-- already been applied.  This keeps record-specific holds from retaining
-- unrelated accounts.
...
begin
  ...
  keep_evidence := public.retention_policy_enabled('moderation_evidence')
    or exists (
      select 1 from public.data_retention_holds h
       where h.category = 'moderation_evidence' and h.released_at is null and h.started_at <= now()
         and (h.record_id is null or h.record_id = any(report_ids) or exists (
  ...
  keep_audit := public.retention_policy_enabled('moderation_audit')
    or exists (
      select 1 from public.data_retention_holds h
       where h.category = 'moderation_audit' and h.released_at is null and h.started_at <= now()
         and (h.record_id is null or exists (
           select 1 from public.moderation_audit_log a
            where a.id = h.record_id
              and (a.moderator_id = me or a.target_user_id = me or a.report_id = any(report_ids))
         ))
    );

  if keep_evidence then
    update public.reports r set target_id = null, target_profile_id = null,
    ...
    delete from public.reports r where r.reporter_id = me or r.id = any(report_ids);
  end if;

  if keep_evidence then
    update public.profile_moderation_evidence set target_user_id = null,
  ...
  if keep_audit then
    update public.moderation_audit_log set moderator_id = null, target_user_id = null,
      metadata = jsonb_build_object('redacted_after_account_deletion', true)
     where moderator_id = me or target_user_id = me or report_id = any(report_ids);
  else
