-- Moderators may review a conversation only through an active case assignment.
-- Administrators retain the existing report/user-context access path.
...
begin
  ...
  -- A report being valid is not sufficient for moderator conversation access.
  -- The report must belong to a live case actively assigned to this moderator.
  -- Expired, resolved, dismissed, unassigned, and stolen claims all fail closed.
  if not admin_access then
    select mcr.case_id
      into assigned_case
      from public.moderation_case_reports mcr
      join public.moderation_cases mc on mc.id = mcr.case_id
     where mcr.report_id = report_uuid
       and mc.assigned_staff_id = auth.uid()
       and mc.claim_expires_at is not null
       and mc.claim_expires_at > now()
       and mc.status not in ('resolved', 'dismissed')
     limit 1;
    if assigned_case is null then raise exception 'An active moderation case assignment is required'; end if;
  end if;
  ...
  insert into public.moderation_audit_log(moderator_id, report_id, target_user_id, action, metadata, case_id) values (auth.uid(), report_uuid, effective_target, 'conversation_review', jsonb_build_object('conversation_id', conversation_uuid, 'reason', clean_reason, 'context', case when report_uuid is null then 'admin_user_detail' else 'moderation_report' end, 'report_id', report_uuid), assigned_case);
  return result;
end;
$$;
