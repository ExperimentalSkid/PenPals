-- Allow protected profile saves to repair legacy rows that predate the
-- normalized location columns.  Older rows can have a canonical country and
-- city while country_code/region_code/locality_id are still null.  Requiring
-- a caller to supply a region in that state made otherwise valid profile
-- edits fail with "A region is required for this precision".
--
-- Only an unambiguous locality-name match is inferred.  Ambiguous or unknown
-- names continue through the existing validation path and are not guessed.
...
begin
  ...
  select exists (select 1 from public.profiles where id = me) into existing_profile;
  ...
  if clean_username !~ '^[a-z0-9_]{3,24}$'
     ...
     or (not existing_profile and char_length(clean_quote) < 1)
  ...
  if clean_country_code is null then
    ...
    if clean_country_code is null then
      -- Keep an unresolved legacy location as text. Child normalized fields
      -- are cleared so the hierarchy guard can safely preserve that value.
  ...
  if clean_country_code is not null then
    if clean_precision = 'country' then
      ...
      -- Legacy rows often retain a city name but no normalized child keys.
      -- Infer only when that city maps to exactly one locality in the
      -- selected country; never select an arbitrary region.
