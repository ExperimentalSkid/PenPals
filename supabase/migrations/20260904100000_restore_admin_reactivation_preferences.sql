-- Preserve the target's pre-deactivation introduction preference when an
-- administrator changes account status. This is account lifecycle state only;
-- voluntary `inactive_mode` remains independent and is not changed here.
