-- Give the site's own @admin profile its requested public title while keeping
-- the participation rank system unchanged for every other member.
...
as $$
  select case
    when p.role = 'admin' and lower(p.username) = 'admin' then
      jsonb_build_object(
