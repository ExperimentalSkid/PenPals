-- A deactivated account cannot use the private-photo authorization RPC as a
-- direct API escape hatch.  Keep voluntary Pause separate: paused accounts
-- remain intact and may retain access to already-established photo grants.
     ...
     and exists (
       select 1
        ...
        where viewer_profile.id = auth.uid()
          and viewer_profile.deactivated_at is null
     )
     and (
       owner_user = auth.uid()
       or exists (
