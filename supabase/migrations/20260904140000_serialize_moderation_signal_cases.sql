-- Serialize automated signal processing for one exact target.  A profile
-- update and a preserved report can arrive at the same time; without a
-- target-level lock both transactions could create duplicate open cases.
-- This only protects case/flag association.  Signals remain review-only.
...
begin
  ...
  -- Keep the case lookup and creation in the same serialized critical
  -- section, while retaining the per-signal lock below for duplicate flags.
  perform pg_advisory_xact_lock(hashtextextended(
    'auto-case:' || coalesce(target_kind, '') || ':' || coalesce(target_uuid::text, ''),
    0
  ));
  ...
  for rule_row in select * from public.moderation_detection_rules where enabled order by id loop
      ...
      select 1 from public.moderation_detection_rules other
       where other.enabled and other.id <> rule_row.id
         ...
         and (
           ...
           or (
             rule_row.context_group in ('promotion','bridge','creator')
             and other.context_group in ('promotion','bridge','creator')
             and rule_row.context_group <> other.context_group
           )
         )
    ...
    select c.id into target_case from public.moderation_cases c
     where c.primary_target_type = target_kind and c.primary_target_id = target_uuid
