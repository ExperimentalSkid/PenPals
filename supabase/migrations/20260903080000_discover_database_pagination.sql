-- Keep discovery filtering, eligibility and ordering in Postgres so the
-- application only receives the requested page.  The existing
-- get_discover_profiles(uuid) RPC remains unchanged for compatibility with
-- callers that still need the unfiltered projection.
...
create index if not exists profile_languages_profile_purpose_idx
