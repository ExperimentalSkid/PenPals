-- Allow existing legacy profiles with an empty quote to update other fields.
-- New profile creation still requires a quote, while the browser form marks
-- the completion fields as required so users receive an actionable message.
