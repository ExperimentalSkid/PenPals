-- Preserve unresolved legacy free-text locations during profile edits.
-- The normalized location rollout intentionally left values such as an
-- internal/legacy country name without a country_code.  Saving any other
-- profile field must not turn that existing, readable value into an error.
