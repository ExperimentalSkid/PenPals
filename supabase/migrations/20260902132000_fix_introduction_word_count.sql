-- Fix introduction word counting without changing any other submission rules.
-- PostgreSQL POSIX whitespace covers spaces, tabs, and newlines.
...
begin
  ...
  if char_length(body) < 50 or char_length(body) > 500 or body !~ '^[[:alnum:]].*[[:alnum:]]$' or (select count(*) from regexp_split_to_table(body, '[[:space:]]+') as words) < 8 then
