-- Correct the response-opportunity timestamp without rewriting the original
-- acceptance migration. The opportunity begins when the introduction was
-- sent, not when the recipient eventually accepts it.
...
begin
  ...
  if existing_conversation_id is null then
    ...
    -- participants' current preference at this new-contact boundary.
    if exists (
      select 1
        from public.profiles
  ...
  -- Preserve the opportunity's original sent time so response metrics and
  -- completion windows remain tied to the introduction, not the acceptance.
